...
import { deleteMessage, publishMessage } from './helpers/requestHelpers'
...
export default class Bot {
  constructor(token) {
    ...
    this.invalidStudentErrorMessage =
      "That's not a student! Use xx00(0)xx :wink:"
  }
  ...
  init() {
    ...
    controller
      ...
      .startRTM()
      .on('direct_message', (bot, message) => {
        this.message(bot, message)
      })
      .on('direct_mention', (bot, message) => {
        this.message(bot, message)
      })
      .on('mention', (bot, message) => {
        this.message(bot, message)
      })
  }
  ...
  async handleClearMessage(bot, message, username) {
    try {
      ...
      bot.reply(
        message,
        'Internal Bot Error - a trained AI has been sent to investigate',
      )
